...
TITLE Hexed Underworld Compiler - Thanks Tiger!
...
START "HexedUW Compile: 7Zip" /B /%4 /WAIT "%ProgramDirPath%tools\7za.exe" a -t%1 -mm=%2 -mx=%3 -x@"%ProgramDirPath%tools\7zExcludeListDir.txt" -xr@"%ProgramDirPath%tools\7zExcludeList.txt" "%ProgramDirPath%..\%projectName%.pk3" "%ProgramDirPath%*"
...
IF %featuresGit% NEQ True (
    ...
    IF "%projectName%" NEQ "hexeduw" SET "projectName=hexeduw"
    GOTO :EOF
)
...
REM Avoid redundancy
IF "%projectName%" NEQ "hexeduw-%GitCommitHash%" SET "projectName=hexeduw-%GitCommitHash%"
GOTO :EOF
